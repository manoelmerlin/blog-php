...
<div>
	<h5> Post criado em :
		<?php echo $this->Time->format($post['Post']['created'], '%d/%m/%Y as %T');?></h5> <h5>Autor :<?php echo $this->Html->link($this->Html->image('../img/profilepic/' . $post['User']['imagem'], array('alt' => 'Brownies', 'style' => 'width:70px; height:50px; border-radius: 25px;', 'class' => 'my-3 ml-1')), array('controller' => 'users', 'action' => 'viewprofile', $post['Post']['created_by']), array('escapeTitle' => false, 'title' => 'Profile pic'
		));?>
	</h5>
...
<div class="my-5">
        ...
        <?php foreach ($comentarios as $a): ?>
            <div class='border p-2 my-4'>
                <div class="my-3">
                    <div class="row m-0">
                        <?php echo $this->Html->link($this->Html->image('../img/profilepic/' . $a['User']['imagem'], array('alt' => 'Brownies', 'style' => 'width:70px; height:50px; border-radius: 25px;', 'class' => 'float-right my-1 ml-1')), array('controller' => 'users', 'action' => 'viewprofile', $a['Comment']['created_by']), array('escapeTitle' => false, 'title' => 'Profile pic'));?>
                            <h6 class="my-3 ml-2"><?php echo h($a['User']['first_name']); ?></h6>  <h6 class="my-3 ml-2">
								<?php echo h($a['Comment']['last_name']); ?>
							</h6>
                    </div>
                    <div>
				...
				<?php echo h($a['Comment']['body_commit']);?>   <h6 class="float-right"> <?php echo 'Comentado : ' . $this->Time->format($a['Comment']['modified'], '%d/
				%m/
				%Y as %T'); ?>
            </div>
								<?php endforeach;
